...
# FCN 8s - Don't use VGG16
class fcn8sNoVgg16(nn.Module):
    def __init__(self, n_classes=21, learned_billinear=True):
        super(fcn8sNoVgg16, self).__init__()
        self.learned_billinear = learned_billinear
        self.n_classes = n_classes
        self.loss = functools.partial(cross_entropy2d, size_average=False)
...
# FCN 8s - Reduce channels by half
class fcn8sHalf(nn.Module):
    def __init__(self, n_classes=21, learned_billinear=True):
        super(fcn8sHalf, self).__init__()
        self.learned_billinear = learned_billinear
        self.n_classes = n_classes
        self.loss = functools.partial(cross_entropy2d, size_average=False)

        self.conv_block1 = nn.Sequential(
            nn.Conv2d(3, 32, 3, padding=100),
            nn.ReLU(inplace=True),
...
# FCN 8s - Reduce channels by 75%
class fcn8sQuarter(nn.Module):
    def __init__(self, n_classes=21, learned_billinear=True):
        super(fcn8sQuarter, self).__init__()
        self.learned_billinear = learned_billinear
        self.n_classes = n_classes
        self.loss = functools.partial(cross_entropy2d, size_average=False)

        self.conv_block1 = nn.Sequential(
            nn.Conv2d(3, 16, 3, padding=100),
            nn.ReLU(inplace=True),
        ...
        self.score_pool4 = nn.Conv2d(128, self.n_classes, 1)
        self.score_pool3 = nn.Conv2d(64, self.n_classes, 1)
...
# FCN 8s - Delete block 3
class fcn8sDelB3(nn.Module):
    def __init__(self, n_classes=21, learned_billinear=True):
        super(fcn8sDelB3, self).__init__()
        self.learned_billinear = learned_billinear
        self.n_classes = n_classes
        self.loss = functools.partial(cross_entropy2d, size_average=False)
    ...
    def forward(self, x):
        ...
        if self.learned_billinear:
            ...
            score_pool3c = self.score_pool3(conv2)[
                :, :, 9 : 9 + upscore_pool4.size()[2], 9 : 9 + upscore_pool4.size()[3]
            ]
            ...
            score_pool4 = self.score_pool4(conv4)
            score_pool3 = self.score_pool3(conv2)
            score = F.upsample(score, score_pool4.size()[2:])
            score += score_pool4
            score = F.upsample(score, score_pool3.size()[2:])
            score += score_pool3
